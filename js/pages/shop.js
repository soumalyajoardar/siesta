...
export function cardHTML(p, i = 0) {
    ...
    return `<article class="p-card reveal" style="transition-delay:${Math.min(i * 40, 320)}ms; display:flex; flex-direction:column; height:100%">
      ...
      <div class="p-body" style="display:flex; flex-direction:column; flex:1">
        ...
        <span class="p-meta">${esc(p.colors.map((c) => c.name).join(" / "))} &middot; ${esc(p.sizes.slice(0, 4).join(", "))}${p.sizes.length > 4 ? "+" : ""}</span>
        <div class="p-price"><span class="price" style="margin-right:.4rem">${off > 0 ? `<span style="color:var(--forest);font-weight:700;margin-right:.25rem">&darr; ${off}%</span>` : ""}${inr(p.price)}</span>${p.mrp > p.price ? `<span class="mrp" style="text-decoration:line-through;color:var(--muted);font-size:.85em">${inr(p.mrp)}</span>` : ""}</div>
...
export function HomePage() {
  ...
  const heroSlideHTML = (b, i) => {
      ...
      const stack = (cls, alt) => photos.map((src, k) => `<img class="${cls}${k === 0 ? " on" : ""}" data-pi="${k}" src="${esc(imgVariant(src, 1200, 70))}" alt="${esc(alt)}"${cls === "hero-fg" && k > 0 ? ' aria-hidden="true"' : ""}${i === 0 && k === 0 && cls === "hero-fg" ? ' fetchpriority="high"' : ' loading="lazy"'} onload="this.classList.add('on');this.closest('.hero-art')?.classList.add('ld')" onerror="this.closest('.hero-art')?.classList.add('ld');this.remove()" style="position:absolute; inset:0; width:100%; height:100%; object-fit:cover;" />`).join("");
      
      // We wrap the art in an inner box to give it the padding and border requested in the sketch.
      return `
        <div class="hero-slide${i === 0 ? " active" : ""}" data-slide="${i}"${i === 0 ? "" : ' aria-hidden="true"'} style="background:#fff;">
          <div class="hero-copy" style="padding: clamp(2.5rem, 5vw, 4rem); display:flex; flex-direction:column; justify-content:center;">
            <span class="eyebrow" style="display:inline-flex; align-items:center; gap:0.6rem; font-size:0.75rem; letter-spacing:0.12em; color:var(--forest-deep); font-weight:700; text-transform:uppercase;">
              ${esc(b.eyebrow || "NEW SEASON · AUTUMN-WINTER 2026")}
            </span>
            <h1 style="font-family:var(--font-display); font-size:clamp(2.5rem, 5vw, 4.2rem); line-height:1.05; margin:1.2rem 0; font-weight:800; color:var(--ink);">${esc(b.title)}</h1>
            <p style="color:var(--muted); font-size:clamp(0.95rem, 1.2vw, 1.1rem); max-width:48ch; margin:0 0 2rem; line-height:1.6;">${esc(b.message)}</p>
            <div class="hero-cta" style="display:flex; gap:1rem; flex-wrap:wrap; margin-bottom:2.5rem;">
              <a class="btn btn-dark" href="/shop?gender=men" style="border-radius:999px; padding:0.8rem 1.6rem; font-weight:700;">Shop Men</a>
              <a class="btn btn-outline" href="/shop?gender=women" style="border-radius:999px; padding:0.8rem 1.6rem; font-weight:700; background:#fff; border-color:var(--line-2);">Shop Women</a>
              <a class="btn btn-outline" href="/shop?gender=unisex" style="border-radius:999px; padding:0.8rem 1.6rem; font-weight:700; background:#fff; border-color:var(--line-2);">Shop Unisex</a>
            </div>
            <div class="hero-meta" style="display:grid; grid-template-columns:repeat(3, 1fr); gap:1rem; font-size:0.85rem; color:var(--muted);">
              <div><strong style="display:block; color:var(--ink); font-size:0.9rem; margin-bottom:0.2rem;">COD available</strong>Pay at your door</div>
              <div><strong style="display:block; color:var(--ink); font-size:0.9rem; margin-bottom:0.2rem;">Easy 7-day returns</strong>No questions asked</div>
              <div><strong style="display:block; color:var(--ink); font-size:0.9rem; margin-bottom:0.2rem;">Ships in 24 hrs</strong>Across India</div>
            </div>
          </div>
          <div class="hero-art-wrap">
            <div class="hero-art"${photos.length ? " data-photo" : ""} style="flex:1; position:relative; min-height:420px; border-radius:2px; overflow:hidden;">
              ${heroArt()}
              ${stack("hero-bg", "")}
              ${stack("hero-fg", b.title)}
            </div>
          </div>
        </div>`;
    };
