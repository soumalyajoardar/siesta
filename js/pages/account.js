...
const needAuth = async () => {
  ...
  if (!u) { window.navigate("/login?next=" + encodeURIComponent(location.pathname + location.search)); return null; }
  return u;
};
...
function wireAccount(tab, u) {
  ...
  if (tab === "overview") {
    ...
    main.innerHTML = `<div class="card"><h3 style="margin-top:0">Saved addresses</h3><div class="addr-grid">${list.map((a) => `<div class="addr-card ${a.isDefault ? "default" : ""}"><strong>${esc(a.name)}</strong> ${a.isDefault ? '<span class="pill">Default</span>' : ""} <span class="muted" style="font-size:.8rem">${addrIcon(a.label)} ${addrLabel(a.label)}</span><br/><span class="muted">${esc(a.line1)}, ${esc(a.city)} ${esc(a.pin)}<br/>${esc(a.phone)}</span><div style="display:flex;gap:.6rem;margin-top:.5rem"><button class="link-btn" data-del="${a.id}">Delete</button></div></div>`).join("") || '<p class="muted">No saved addresses yet.</p>'}</div>
    <form id="addrMini" class="form-grid" style="margin-top:1rem"><div class="field"><label>Full name *</label><input name="name" class="input" required/></div><div class="field"><label>Phone *</label><input name="phone" class="input" required/></div><div class="field full"><label>Address *</label><input name="line1" class="input" required/></div><div class="field"><label>City *</label><input name="city" class="input" required/></div><div class="field"><label>PIN *</label><input name="pin" class="input" required/></div><div class="field"><label>Label</label><select name="label" class="select"><option value="home">Home</option><option value="work">Work</option></select></div><div class="full"><button class="btn btn-dark" type="submit">Save Address</button></div></form></div>`;
