...
import { BUSINESS, LAUNCH_CHECKLIST } from "../config.js";
...
export function ForgotPage() {
  ...
  setTimeout(() => {
    document.getElementById("fgForm").onsubmit = (e) => {
      ...
      toast("If an account exists for this email, reset instructions would be sent. (Demo: password reset requires a backend email service.)");
      location.hash = "#/login";
    };
  });
...
function wireAccount(tab, u) {
  ...
  if (tab === "overview") {
    ...
    main.innerHTML = `<div class="card"><h3 style="margin-top:0">Order history</h3>${orders.length ? orders.map((o) => `<div class="order-card"><div class="order-top"><div><strong>${esc(o.orderNo)}</strong><br/><span class="muted" style="font-size:.84rem">${new Date(o.createdAt).toLocaleDateString("en-IN")} · ${o.items.reduce((s, i) => s + i.qty, 0)} items · ${inr(o.amounts.total)} · COD</span></div><span class="pill ${o.status === "delivered" ? "ok" : o.status === "cancelled" ? "" : "warn"}">${esc(o.status.replace(/_/g, " "))}</span></div>
      ...
      <div data-dwrap="${esc(o.orderNo)}" hidden style="margin-top:.6rem">${o.items.map((i) => `<div class="summary-row"><span>${esc(i.name)} × ${i.qty} (${esc(i.size)})</span><span>${inr(i.price * i.qty)}</span></div>`).join("")}</div></div>`).join("") : `<div class="empty"><h2>No orders yet</h2><p class="muted">Orders placed on this device will appear here.</p><a class="btn btn-dark" href="#/shop">Start Shopping</a></div>`}</div>`;
...
export const StaticPages = {
  ...
  privacy: () => prose("Privacy Policy", `<p>We collect only what we need: account details you provide, delivery addresses, order contents, and your cookie choices. We do not collect card numbers, CVVs, UPI PINs or banking credentials — the checkout never asks for them.</p><h2>What we store and where</h2><p>In this build, data is stored <strong>only in your own browser</strong> (local storage): cart, wishlist, hashed account credentials, addresses and orders. There is no Siesta server yet. Before launch this must move to a secured backend with encryption in transit and at rest.</p><h2>Marketing consent</h2><p>Newsletter and marketing checkboxes are never pre-checked. You can withdraw consent anytime from Account → Preferences.</p><h2>Your rights</h2><p>Access, correct or erase your data from Account settings (“Erase my data on this device”). For server-side copies after backend launch, contact the grievance officer at ${esc(BUSINESS.grievanceOfficer)}.</p><h2>Third parties</h2><p>We load no advertising trackers, analytics pixels, chat widgets or social embeds in this build. If any are added later, they will be listed here and gated behind cookie consent.</p>`),
